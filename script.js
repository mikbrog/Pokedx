...
const myFavoritePokemon = ["mudkip", "vaporeon", "gardevoir", "lucario", "lopunny","sylveon", "bisharp", "rayquaza", "ledyba", "growlithe", "jynx"];
...
async function loadPokedex() {
  const myFavoritePokemon = ["mudkip", "vaporeon", "gardevoir", "lucario", "lopunny","sylveon", "bisharp", "rayquaza", "ledyba", "growlithe", "jynx"];
  const urls = myFavoritePokemon.map(name => `https://pokeapi.co/api/v2/pokemon/${name}`);
  const details = await Promise.all(urls.map(url => fetchJson(url)));
